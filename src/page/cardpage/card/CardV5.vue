<template>
	<div class="card-v5b-container" @click="onClick">
		<div v-if="isTop" class="v5-content-wrapper">
			<div
				class="photo-wrapper"
				v-bind:style="getPhotoStyle"
				v-bind:class="{ 'top-margin': isTop }"
			>
				<img
					class="photo"
			...
			<div class="content-wrapper">
				<label class="title" style="-webkit-box-orient:vertical">{{
					getTitle
				}}</label>
				<div class="bottom-wrapper">
					<div class="desc-wrapper">
						<label class="subtitle" style="-webkit-box-orient:vertical">{{
							getSubtitle
						}}</label>
						<label class="desc" style="-webkit-box-orient:vertical">{{
							getDesc
						}}</label>
					</div>
					<div
		...
		<div v-else class="v5-content-wrapper">
			...
			<div
				class="photo-wrapper"
				v-bind:style="getPhotoStyle"
				v-bind:class="{ 'bottom-margin': !isTop }"
			>
				<img
					class="photo"
					:onerror="defaultImage"
	...
	computed: {
		...
		isShowSellOutIcon() {
			if (this.isProductItem()) {
				if (
					this.item.product.good_stock &&
					parseInt(this.item.product.good_stock) > 0
				) {
					return false
				} else {
					return true
		...
		isShowPromoIcon() {
			if (this.isProductItem()) {
				if (
					this.item.product.activity &&
					typeof this.item.product.activity === 'object'
				) {
					return true
				} else {
					return false
		...
		getSubtitle: function() {
			...
			if (this.isProductItem()) {
				...
				subtitle = '￥ ' + this.utils.currencyPrice(subtitle)
			} else {
