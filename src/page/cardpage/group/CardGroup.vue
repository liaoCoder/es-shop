<template>
	<div class="group-container">
		<group-section class="group-section" :item="item" v-if="isShowGroupSection">
		</group-section>
		<div class="group-content">
			<card-group-a3-x-x-h
				v-if="isCardGroup('A3XXH')"
				:item="item"
			></card-group-a3-x-x-h>
...
export default {
	...
	computed: {
		...
		isShowGroupSection() {
			let isShow = false
			if (
				this.isCardGroup(ENUM.CARDGROUP_LAYOUT.C1H) ||
				this.isCardGroup(ENUM.CARDGROUP_LAYOUT.C3S)
			) {
				isShow = false
			} else {
