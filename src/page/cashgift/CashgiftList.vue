<template>
	<div class="container">
		...
		>
			<cashgift-item v-for="(item, index) in items" :key="index" :item="item">
			</cashgift-item>
		</base-list>
		<empty-item
...
import { cashgiftList } from '../../api/cashgift'
export default {
	components: {
		CashgiftItem
	},
	...
	methods: {
		...
		loadPageData(isFirstPage) {
			...
			if (this.currentIndex === 0) {
				status = ENUM.CASHGIFT_STATUS.AVAILABLE
			} else if (this.currentIndex === 1) {
				status = ENUM.CASHGIFT_STATUS.USED
			} else if (this.currentIndex === 2) {
				status = ENUM.CASHGIFT_STATUS.EXPIRED
			}
			cashgiftList(page, per_page, status).then(
				res => {
					if (res && res.cashgifts) {
						if (isFirstPage) {
							this.items = res.cashgifts
						} else {
							this.items = this.items.concat(res.cashgifts)
						}
