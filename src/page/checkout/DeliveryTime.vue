		...
		<div class="list-wrapper">
			<div class="list leftList">
				<div
					class="item-wrapper"
					v-bind:class="{
						itemSelected: isSelectedDate(item),
						itemNormal: !isSelectedDate(item)
					}"
				...
				>
					<label
						class="date"
						v-bind:class="{
							dateSelected: isSelectedDate(item),
							dateNormal: !isSelectedDate(item)
						}"
					>
						{{ item.date }}
					</label>
			...
			<div class="list rightList">
				...
				>
					<label
						class="time"
						v-bind:class="{
							timeSelected: isSelectedTime(item),
							timeNormal: !isSelectedTime(item)
						}"
						>{{ item }}</label
					>
					<img
						v-if="isSelectedTime(item)"
						class="indicator"
						src="../../assets/image/change-icon/d1-yes@2x.png"
					/>
				</div>
			</div>
		</div>
